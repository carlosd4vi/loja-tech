...
export default function DashboardProdutos() {
  ...
  useEffect(() => {
    const buscarDados = async () => {
      try {
        ...
        // ORDENAÇÃO DEFINITIVA PELA DATA
        // Usando o campo exato da sua imagem: created_at
        const produtosRecentes = data.sort((a, b) => {
  ...
  const executarExclusao = async () => {
    ...
    try {
      ...
      // DICA EXTRA: Após excluir, é bom recarregar a lista ou remover o item do estado atual
      // para que ele suma da tela sem precisar dar F5.
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {/* NOVO: map direto no array, sem o reverse() */}
            {produtos.map((produto) => (
              <tr key={produto.id} className="hover:bg-gray-50 dark:hover:bg-[#374151]/50 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded bg-gray-100 flex items-center justify-center overflow-hidden">
                      {/* DICA EXTRA: adicionei object-cover pra imagem não distorcer */}
                      <img src={produto.Imagem} alt={produto.nomeProduto} className="w-full h-full object-cover" />
                    </div>
